...
exports.allStudentsByNameInvite = async (req, res) => {
  ...
  try {
    ...
    if(result.length === 0) {
      return res.status(404).json({ Error: "Estudantes não encontrados" });
    }

    res.status(200).json({ message: "Informações dos estudantes", result });
  } catch (err) {
    console.error("Erro ao buscar as informações do estudante:", err);
    res.status(500).json({ Error: "Erro ao buscar as informações do estudante" });
  }
};
